...
PerzeptronMtx::PerzeptronMtx(int in_hidden_layers, int in_first_layer) : m_weights(new std::vector<NeuronMatrix>()) {
  if (in_hidden_layers > 5 || in_hidden_layers < 1)
    throw MyException("incorrect input for hidden layer");
...
PerzeptronMtx::PerzeptronMtx(PerzeptronMtx &&other) noexcept { *this = other; }

PerzeptronMtx::PerzeptronMtx(const PerzeptronMtx &other) { *this = other; }
...
PerzeptronMtx::~PerzeptronMtx() { delete m_weights; }
...
double PerzeptronMtx::getMiddleErr(const std::pair<int, NeuronMatrix> &in_data) {
  ...
  for (int i = 0; i < in_data.second.getColum(); ++i)
    result += in_data.first - 1 == i ? (1 - in_data.second(0, i))
                                     : in_data.second(0, i);
...
std::pair<int, std::vector<double>> PerzeptronMtx::learn(const std::string &in_path, int in_epochs) {
  ...
  while (counter--) {
    for (auto &&el : base)
      tasks.push_back(
          pool.enqueue(prediction, el.second, m_weights, k_step, el.first));
  }
...
std::pair<int, std::vector<double>> PerzeptronMtx::crossValidation(const std::string &in_path, int in_k) {
  if (in_k <= 0) throw MyException("incorrect count epoch");
  auto base = Parser::parseDataBase(in_path, 1.0);
  if (in_k > base.size())
  ...
  for (int i = 0; i < (int)parts.size(); ++i) {
    ...
    for (auto &&el : parts[i])
      test_tasks.push_back(pool.enqueue(analyzeCase, &pass, el.first, el.second,
                                        m_weights, k_step, &analytic, &mute));
...
analytical_data PerzeptronMtx::test(const std::string &in_path, double in_data_part) {
  ...
  std::mutex mute;
  for (auto &&el : base)
    tasks.push_back(pool.enqueue(analyzeCase, &pass, el.first, el.second,
                                 m_weights, k_step, &analytic, &mute));
  for (auto &&el : tasks) el.get();
  timer.stop();
...
std::pair<int, NeuronMatrix> PerzeptronMtx::prediction(const NeuronMatrix &in_layer,
  ...
  if (in_correct) {
    ...
    for (auto &&el : layers) errors.emplace_back(std::move(*(el.transport())));
    for (int i = 0; i < 26; ++i) {
      double o_i = layers.back()(0, i);
      double err_i =
          -o_i * (1 - o_i) * ((i + 1 == in_correct.value() ? 1. : 0.) - o_i);
    ...
    for (auto &&el : *inout_weights) el -= *(minus++);
  }
  return std::make_pair(num + 1, layers.back());
}

void PerzeptronMtx::analyzeCase(int *out_count,
                                int in_correct,
  ...
  if (in_correct != control.first) *out_count -= 1;
}
