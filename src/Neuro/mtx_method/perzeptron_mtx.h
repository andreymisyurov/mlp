...
class PerzeptronMtx : public Perceptron {
 ...
 public:
  ...
  // private:
 public:
  /** СТАТИЕСКИЙ МЕТОД prediction. Является основным методом класса,
   * задействован в методах learn, test, predict. Реализован с использованием
   * многопоточности.
   *
   * @param in_layer матрица значений входного слоя вида 1x784
   * @param inout_weights указатель матрицу значений весов, веса меняются при
   * обучении
   * @param in_step шаг обучения
   * @param in_correct ожидаемый ответ, значение int от 0 до 25 включительно
   * @return возвращает пару, где first - это значение от 1 до 26
   * включительно.\n Предсказание сети(номер нейрона выходного слоя с наибольшим
   * зарядом),\n а second - матрица всех зарядов нейронов выходного
   * слоя(необходимо для аналитики)
   */
  ...
  /** СТАТИЕСКИЙ МЕТОД analyzeCase - анализирует поведение нейронной сети,
   * собирает аналитику.
   *
   * @param out_count результирующи аргумент, изначально равен количеству
   * тест-кейсов. Изменяется в потоках
   ...
   * @param in_step шаг обучения
   * @param out_analytic вектор с аналитическими данными, который в потоках
   * накапливает следующие значения:\n
   ...
   * @param in_mutex мьютекс для контроля доступа к аргументам out_count и
   * out_analytics
   */
  auto static analyzeCase(int *out_count, int in_correct,
                          const NeuronMatrix &in_mtrx,
                          const std::vector<NeuronMatrix> *in_weights,
                          double in_step, std::vector<int> *out_analytic,
                          std::mutex *in_mutex)                                 -> void;
  /** СТАТИЕСКИЙ МЕТОД feel_random заполняет матрицу случайными значениями от
   * -0.5 до 0.5
   *
   * @param out_mtrx результирующий аргумент, матрица которую нужно заполнить
   ...
   * слое
   *
   * @param in_data матрица выходного слоя состоящая из 26 значений зарядов
   * нейронов
