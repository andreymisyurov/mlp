...
void Assistant::showWidget(const QString &in_path, std::optional<QMessageBox::Icon> in_widget,
                                                std::optional<QString> in_title, std::optional<QString> in_message) {
  m_effect.setSource(QUrl::fromLocalFile(in_path));
  m_effect.play();
  if(in_widget)
    QMessageBox(in_widget.value(), in_title.value(), in_message.value(), QMessageBox::StandardButton::Apply).exec();
}
...
void Assistant::createGraphic(const std::pair<int, std::vector<double>> &in_data, int in_type) {
...
auto Assistant::createAnalyticString(const analytical_data &in_date) -> QString {
  QString result = "Average accuracy = " + QString::number(in_date.average_accuracy) + '\n' + "Precision = "
      + QString::number(in_date.precision) + '\n' + "Recall = " + QString::number(in_date.recall) + '\n'
