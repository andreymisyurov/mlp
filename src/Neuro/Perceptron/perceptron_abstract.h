 ...
 * precision - количество объектов, которые сеть указала как положительные и
...
class Perceptron {
 public:
  /** МЕТОД learn. Функция обучает нейросеть, и возвращает график с изменением
   * ошибки во время обучения принимает путь к данным в формате .csv\n Нейросеть
   * спроектирована так что данные в базе должны быть значения int, записанные
   * через запятую. одна строчка - это один кейс\n
   *
   ...
   * учить 2 эпохи по 88.000 элементов функция должна вернуть пару где первый
   * элемент равен 2, а второй - это вектор из 176.000 элементов средних ошибок
   * на каждый кейс
   */
  virtual auto learn(const std::string &in_path, int in_epochs)           -> std::pair<int, std::vector<double>> = 0;

  /** МЕТОД crossValidation. Способ обучения при которая база даных разбивается
   * на in_k количество частей. Особым образом обучается и тестируется на этих
   * кусках базы.
   *
   * @param in_path путь к файлу с обучающим материалом
   ...
   * То есть если коэффициент равен 3 то график должен отображать изменение
   * ошибки на протяжении обучения по каждому шагу кросс валидации
   */
   ...
   *
   * @return структура с заполненными аналитическими данными analytical_data
   * описано отдельно сверху
   */
  virtual auto test(const std::string &in_path, double in_data_part)
                                                                          -> analytical_data = 0;

  /** МЕТОД predict. Принимает нейроны входного слоя(в виде матрицы) и выполняет
   * разовое предсказание, в соответствии с весами и возвращает номер нейрона
   * выходного слоя, не изменяет веса.
   *
   * @param in_layer принимает NeuronMatrix, матрицу со значениями входного
   * слоя. Матрица вида 1x784
   * @return возвращает порядковый номер предсказанной буквы от 1 до 26
   */
  virtual auto predict(const NeuronMatrix &in_layer)                      -> int = 0;

  /** МЕТОД exportDataBase. Выгружает веса в определенном виде. Образец для
   * хранения данных следующий:\n 2\n 768 128\n d a t a ....\n 128 26\n d a t a
   * ....\n\n
   *
   * 2 - первая строчка только количество внутренних слоев одной цифрой от 1 до
   * 5\n
   *
   * 768 128 - дальше пошли цифры с количествами нейронов на слоях веса между
   * которыми мы сохраняем, в моем случае это марицы весов, в графовом методе -
   * это веса в графах.\n
   *
   * d a t a .... - дальше сами данные в одну строчку без переносов строки с
   * этого слоя(!) все данные через пробел данные идут подряд одной "колбаской".
   * Сначала все веса первого нейрона первого слоя, потом все веса второго
   * нейрона и тд все слои\n
   *
   * дальше по аналогии\n
   * 128 26\n
   * d a t a ....\n
   * Таких пар строк столько же сколько и слоев. Файл при экспорте должен
   * открываться как бинарный .bin
   *
