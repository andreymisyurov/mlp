...
void LearnAnswer::drawGraph(const std::pair<int, std::vector<double>> &in_data, int in_type) {
  // переносим данные из std::vector в QVector, добавляем один элемент, соединяющий с нулем
  QVector<double> val_err(in_data.second.begin(), in_data.second.end());
  if(in_type) val_err.push_front(*in_data.second.begin());
  // также добавляем нулевую точку к точкам значений абсцисс
  QVector<double> val_epochs((long long)in_data.second.size() + in_type);
  // вычисляем шаг и инициализируем начальное значение. то что слева от дробной черты это то какая
  // целочисленная цифра элементов будет на оси абсцисс
  double step = 1. * (in_data.first + in_type)/(double)val_epochs.size();
  double value = 0.;
  ...
  m_ui->w_qplot->graph(0)->setPen(QPen(Qt::red, 1, Qt::SolidLine, Qt::RoundCap));
  m_ui->w_qplot->graph(0)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssDiamond, 2.0));
  m_ui->w_qplot->replot();
}
