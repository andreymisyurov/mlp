...
class MyException : std::exception {
 public:
  ...
  [[nodiscard]] const char *what() const noexcept override { return m_text.c_str(); }
...
class Matrix {
 public:
  virtual auto trans()                                            -> std::shared_ptr<Matrix<T>>{
 ...
 public:
  [[nodiscard]] auto getColum() const         -> int { return m_column; };
  [[nodiscard]] auto getRow() const           -> int { return m_row; };
  auto setRow(int new_row)                    -> void;
  auto setColum(int new_column)               -> void;
 ...
 public:
  ...
  [[maybe_unused]] virtual auto transpo()     -> Matrix<T> *;
 ...
 protected:
  auto static copyMatrix(int row, int colum,
...
Matrix<T>::Matrix(int value_row, int value_column) {
  checkConstructor(value_row, value_column);
...
template <typename T>
[[maybe_unused]] auto Matrix<T>::transpo() -> Matrix<T> * {
