...
class PerzeptronGrph : public Perceptron {
 ...
 private:
  /** МЕТОД prediction - делает предсказания и корректировки весов(методом
   * обратного распространения ошибки).\n Содержит в себе forward-propagation и
   * back-propagation.\n Основной метод класса, используется в функциях learn,
   * test, predict
   *
   * @param in_layer входящий слой из 784 значений нейронов в диапазоне от 0
   * до 1.
   * @param in_correct ожидаемый ответ сети, значение int от 0 до 25
   * включительно
   * @return возвращает пару, где first - это предсказание сети(номер нейрона
   * выходного слоя с наибольшим\n зарядом от 0 до 25), second - матрица всех
   * зарядов нейронов выходного слоя(необходимо для отчета об обучении)
   */
  auto prediction(const NeuronMatrix &in_layer,
                            std::optional<int> in_correct)                  -> std::pair<int, std::vector<double>>;

  /** МЕТОД analyzeCase - анализирует поведение нейронной сети, собирает
   * аналитику.
   *
   * @param out_count результирующи аргумент, изначально равен количеству
   * тест-кейсов.
   * @param in_correct ожидаемый ответ сети
   * @param in_mtrx матрица 26ти зарядов нейронов выходного слоя
   * @param out_analytic результирующи аргумент - вектор с аналитическими
   * данными, который накапливает следующие значения:\n
   ...
   */
  auto analyzeCase(int *out_count, int in_correct,
              const NeuronMatrix &in_mtrx, std::vector<int> *out_analytic)         -> void;

  /** МЕТОД backPropagation нужен для корректировки весов методом обратного
 ...
 */
  ...
  /** СТАТИЧЕСКИЙ МЕТОД figureOutCharge вычисляет заряд исходя из зарядов и
   * значений весов предыдущего слоя
   *
  ...
  /** СТАТИЧЕСКИЙ МЕТОД находит произведение строки весов входного нейрона на
   * столбец ошибок выходных нейронов\n используется в методе обратного
   * распространения ошибки
   *
  ...
  /** СТАТИЕСКИЙ МЕТОД getMiddleErr высчитывает и усредняет ошибку на выходном
   * слое
   *
   * @param in_data пара из правильного ответа и вектора выходного слоя
   * состоящая из 26 значений зарядов нейронов
 ...
 private:
  ...
  /** СТРУКТУРА GraphData является вложенной и используется для хранения данных
   * исключительно внутри класса. Содержит заряд нейрона и вектор с весами и
   * связями
   */
  struct GraphData {
    double m_charge;
