...
class PerzeptronGrph: public Perceptron {
  ...
  struct GraphData;
 public:
  explicit PerzeptronGrph(int in_hid_layers = 2, int in_first_layer = 256);
 ...
 private:
   ...
   * @param inout_analytic вектор с аналитическими данными, который накапливает следующие значения:\n
   ...
   */
  auto analyzeCase(int *inout_count, int in_correct, const NeuronMatrix &in_mtrx, std::vector<int> *inout_analytic) -> void;
 ...
 * @param num порядковый номер нейрона на выходящем слое
 * @return заряд нейрона под номером num выходного слоя, вещественное число
 */
  auto static figureOutCharge(const std::vector<GraphData> &in_layer, int num) -> double;
   ...
   */
  auto static getErrorRow(const std::shared_ptr<std::vector<double>>& in_weights, const std::vector<double> &in_err) -> double;
 ...
 private:
  ...
  struct GraphData {
    /** СТРУКТУРА GraphData явзяется вложенной и используется для хранения данных исключительно внутри класса.
